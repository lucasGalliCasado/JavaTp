...
public class maxHeap{
    ...
    public void encolar(Tupla value) {
        // O(1)
        if (size == cota) {
        ...
        // O(1)
        heap[size] = value.primerElemento();
        ...
        while (posActual > 0 && heap[posActual].primerElemento() > heap[padre(posActual)].primerElemento()) {
    ...
    public int mirarSegundo(){
        int res;
        if(heap[hijoIzquierdo(0)].primerElemento() > heap[hijoDerecho(0)].primerElemento()){
    ...
    public void array2heap(int[] array){
        ...
        Tupla[] arrayConId = new array(array.length);
        for(int i = 0; i < array.length;i++) {
            arrayConId[i] = Tupla(array[i],i);
        }
    ...
    private void maxHeapify(int nodoActual) {
        ...
        if (hIzq < size && heap[hIzq].primerElemento() > heap[max].primerElemento()) {
            max = hIzq;
        }
        if (hDer < size && heap[hDer].primerElemento() > heap[max].primerElemento()) {
            max = hDer;
        }
