...
public class SistemaCNE {
    ...
    public void registrarMesa(int idMesa, VotosPartido[] actaMesa) {
        ...
        for(int j = 0; j < votosDiputados.length-1;j++){
            filaId[j] = votosDiputados[distrito][j];
        }
        //heapifiamos
    ...
    public int[] resultadosDiputados(int idDistrito){
        ...
        int[] contadorCantidadDeBancas = new int[nombresPartidos.length];
        int k = 0;
        Tupla max = new Tupla(-1,-1);
        while(k < diputadosPorDistritos[idDistrito]){
            max = mirarMax(votosDiputadosXDistHeap[idDistrito]);
            contadorCantidadDeBancas[getSegundoElemento(max)] = contadorCantidadDeBancas[getSegundoElemento(max)] + 1;
            votosDiputadosXDistHeap[idDistrito].extraerMax();
            votosDiputadosXDistHeap[idDistrito].encolar(Tupla((votosDiputados[idDistrito][getPrimerElemento(max)])/contadorCantidadDeBancas[getSegundoElemento(max)],getSegundoElemento(max)));
            k = k++;
            }
